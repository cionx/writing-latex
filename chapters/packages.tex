...
% TODO: Write something about unicode-math
...
The package~\packname{amsmath}\massindex[packages]{amsmath}[\packname] is a standard for mathematical typesetting.
The package~\packname{mathtools}\massindex[packages]{mathtools}[\packname] is an extension of~\packname{amsmath} that fixes some problems and also provides new (and often times very useful) functionalities.
...
New environments can then be defined with the command~\comname{newtheorem}\massindex[\piname{amsthm}]{newtheorem}[\comname]:
...
To define the now counter we use the appropriately named command~\comname{newcounter}\massindex{newcounter}[\comname]:
...
In practice one often wants the counter to be bound to the surround section (or chapter).
...
\end{corollary}
\end{showlatex}
The addition of \inlinecode{[section]} to the definition of the new counter ensures that the resulting counter~\inlinecode{sometheorems} resets every time the counter~\inlinecode{section} is increased (which happens every time a new section begins).
...
\subsubsection{The basic \comname{label} and \comname{ref}}
...
\end{showlatex}
In praxis this behavior can be automated by using the upcoming \packname{cleveref}\massindex[packages,referencing]{cleveref}[\packname] package.
...
These problems can be circumvented by using the package~\packname{cleveref}\massindex[packages,referencing]{cleveref}[\packname].
...
\end{lemma}
The above \lcnamecref{weak cayley} does directly follow of Cayley’s~theorem.
\end{showlatex}
...
The general rule is that the package~\packname{hyperref} should be the very last package to be included, to ensure that it interacts properly with all other used packages.
...
\end{showlatex}
The command~\comname{enquote} can automatically adjust the quotation marks to the conventions of the used language when this language is specified through the package~\packname{babel}\massindex[packages]{babel}[\packname].
This is done by loading the package~\packname{csquotes} with the option~\optname{babel}\massindex[\piname{cleveref}]{babel}[\optname] set to~\optname{true}.
...
The package~\packname{enumitem}\massindex[packages]{enumitem}[\packname]\expandafter\index\expandafter{\einame{enumerate}!zzzz@\igobble |seealso{\packname{enumitem}}} is immensely useful for the configuration of the style and behavior of these list environments.
It provides (among others) the following features:



\subsection{Style of numbering}
...
\subsection{Resuming lists}    

One can resume lists with the option~\optname{resume*}\massindex[\piname{enumitem}]{resume*}[\optname].
The option~\optname{resume}\massindex[\piname{enumitem}]{resume}[\optname] works similarly to~\optname{resume} but doesn’t copy any style options from the previous list.
\begin{showlatex}{Resuming lists with \optname{resume*}}
...
One can change the various spacings\index{spacing}\index{spacing!in lists} involved in the list environments.
...
\index{paragraphs}\index{indentation!of paragraphs}
...
\begin{showlatex}*{Indentation and distance of pararaphs in lists, default behavior}
\begin{enumerate}
  \item
...
One can adjust the indentation of the paragraphs with the option~\optname{listparindent}\massindex[\piname{enumitem}]{listparindent}[\optname] and the additional vertical spacing between paragraphs with the option~\optname{parsep}\massindex[\piname{enumitem}]{parsep}[\optname].
...
One can control the distance between list items with the option~\optname{itemsep}\massindex[\piname{enumitem}]{itemsep}[\optname]:

\begin{showlatex}{Distance between list items with \optname{itemsep}}
...
\index{indentation!for list environments|(}

The environment~\envname{enumerate}\massindex{enumerate}[\envname] has by default two kinds of indentation\massindex{indent}:
...
\begin{showlatex}*{\envname{enumerate}~environment with standard indentation}
...
We can remove both indentations by using the option~\optname{wide}\massindex[\piname{enumitem}]{wide}[\optname] with the value~\optname{0pt}:
\begin{showlatex}*{\envname{enumerate}~environment without  indentation}
Lorem ipsum dolor sit amet,
...
\end{enumerate}
\end{showlatex}
We can add back the identation between the item~labels and the item~text by also setting the option~\optname{leftmargin}\massindex[\piname{enumitem}]{leftmargin}[\optname] to the value~\optname{*}:
\begin{showlatex}*{\envname{enumerate}~environments with only identitation between label and text}
Lorem ipsum dolor sit amet,
...
\end{enumerate}
\end{showlatex}
One can also add back the indentation between the left~margin and the item~labels by also using the option~\optname{labelindent}:
\begin{showlatex}*{\envname{enumerate}~environment with indentation between margin and label}
Lorem ipsum dolor sit amet,
...
\begin{showlatex}*[label = {better standard indentation}]{\envname{enumerate}~environment with indentation between margin and label, and label and text}
Lorem ipsum dolor sit amet,
...
\end{enumerate}
\end{showlatex}
But \cref{better standard indentation} does actually have an advantage over the standard indentation, namely that it properly handels long labels:
...
The indentitation for the environment~\envname{itemize}\massindex{itemize}[\envname] can be adjusted in the same way as for the environment~\envname{description}.

For the environment~\envname{description}\massindex{description}[\envname] we can proceed similarly as for~\envname{enumerate}.
...
\begin{showlatex}*{\envname{description}~environment with standard indentation}
...
\end{description}
\end{showlatex}
We can remove the identation by again using the option~\optname{wide}\massindex[\piname{enumitem}]{wide}[\optname].
\begin{showlatex}*{\envname{description}~environment with no indentation}
Text, text, text, text, text, text, text.
...
\begin{myitemize}
  \item
    A~\filename{bib}-file\massindex[bibliography]{bib-file@\string\filename{bib}-file} that contains the metadata about the literature which should be cited.
  \item
...
\end{myitemize}
One should choose~\packname{biblatex}\massindex[packages,bibliography]{biblatex}[\packname] for the bibliography package and~\appname{biber}\massindex[bibliography]{biber}[\appname] for the back~end program.
...
Instead complains to \packname{biblatex} by changing the appropriate settings.
...
% TODO: Use location instead of abusing publisher.

\subsubsection{Entry for a single book}

The \filename{bib}-file entry for a single book should roughly look as follows: 
...
\begin{mydescription}
  ...
  \item[\optname{author}]
    ...
    \begin{myitemize}
      \item
        ...
        This is required so that~\appname{biber} can properly process this data.
      \item
  ...
  \item[\optname{year}]
    ...
    One can also specify a moth with the key~\optname{month}.
    The values for both keys are expected to be numbers.
    
    One can also use the key~\optname{date}\massindex[\piname{biblatex}!keys]{date}[\optname] to specify the date of publication.
    This key expects a single argument, which is of one of the forms~\optname{year},~\optname{year-month}, or~\optname{year-month-day}.
...
\DeclareFieldFormat[book]{pagetotal}{#1~\ppno}
  ...
  \item[\optname{number}]
    ...
    If you copy your bibliography data from somewhere else then there is a high chance that instead of the key~\optname{number} the key~\optname{volume} is used.
    This is relict from the past that isn’t correct with \packname{biblatex}.
  \item[\optname{isbn}]
...
To cite an article published on arXiv one shouldn’t use the key~\optname{url} because \packname{biblatex} has a built-in way of dealing with arXiv~articles\massindex[bibliography]{arXiv}.
...
\end{showcode}
% TODO: Fix cref-number for Example
The output in the bibliography (see \cref{example bibliography}) will look as follows:
...
\bibliography{name of bib-file}
  ...
  \item
    We set the option~\optname{bibliography}\massindex[KOMA-Script classes, document class!options, bibliography]{bibliography}[\optname] of~\comname{documentclass} to~\optname{totoc} to add the bibliography to the table of contents. (see \cref{layout for scrbook}).
\end{myitemize}
...
\subsubsection{The table itself}
...
\begin{myitemize}
  \item
    This package provides the commands~\comname{toprule}\massindex[\piname{booktabs},tables]{toprule}[\comname], \comname{midrule}\massindex[\piname{booktabs},tables]{midrule}[\comname] and \comname{bottomrule}\massindex[\piname{booktabs},tables]{bottomrule}[\comname] as replacements for \comname{hline}.
