...
One can control the distance between list itemns with the option~\optname{itemsep}\massindex[\piname{enumitem}]{itemsep}[\optname]:
...
\end{enumerate}
\end{showlatex}
We can add back the identation between the item~labels and the item~text by setting the option~\optname{leftmargin}\massindex[\piname{enumitem}]{leftmargin}[\optname] to the value~\optname{*}:
...
\begin{showlatex}*{\envname{enumerate}~environment with indentation between margin and label, and label and text}
Lorem ipsum dolor sit amet,
...
The identitation for the environment~\envname{itemize}\massindex{itemize}[\envname] can be adjusted in the same way as for the environment~\envname{description}.
