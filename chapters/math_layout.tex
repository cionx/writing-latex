...
The method~\inlinecode{\$\$  \$\$} is too low level, and the environment~\envname{eqnarray} has too many problems and has been deprecated since forever.
...
Both commands do the same thing (when the package~\packname{amsmath}\massindex[packages]{amsmath}[\packname] is loaded).
...
\end{showlatex}
The non-starred version~\texttt{equation}\massindex[display math environment, tagging and numbering]{equation}[\envname] numbers the equation.
...
\subsection{The environment~\texttt{gather*}}

The environment~\texttt{gather*}\massindex[display math environment, multi-line mathematics]{gather*}[\envname] is meant for multiple lines that are non-aligned.
...
\end{gather*}
\end{showlatex}
The non-starred version~\envname{gather}\massindex[display math environment, multi-line mathematics, tagging and numbering]{gather}[\envname] numbers the lines.
\begin{showlatex}{Using the environment~\envname{gather}}
...
The environment~\envname{align*}\massindex[display math environment, multi-line mathematics]{align*}[\envname] takes multiple lines which can aligned.
...
\end{alignat*}
\end{showlatex}
For aligning multiple columns one should use \envname{alignat*} instead of \envname{align*} to get (manually) a good looking distance between the aligned columns.
The environment~\envname{alignat}\massindex[display math environment, multi-line mathematics]{alignat}[\envname] works the same way as \envname{alignat*} but automatically numbers the lines.
...
\begin{figure}[tb]
  ...
  \begin{tikzpicture}[
    ...
    \node (equation)
          ...
          {\texttt{{\tbs}[ {\tbs}]}\\\inlinecode{equation*}};
    \node (gather)
...
\end{showlatex}
The empty pair of curly brackets in \texttt{{\tbs}iff\{\}\&} ensures that the spacings coming from \comname{iff} and \texttt{\&} do not interfere with each other.
...
The one-line display environments~\inlinecode{{\tbs}[ {\tbs}]}\massindex[display math environment]{{\tbs}[ {\tbs}]}[\inlinecode] and \massindex[display math environment]{equation*}[\envname] do therefore automatically adjust this spacing.
...
The multi-line display mode environments (like~~\envname{align*}\massindex[display math environment, multi-line mathematics]{align*}[\envname]) don’t have this feature.
It always uses the spacing~\comname{abovedisplayskip} (for technical reasons).
...
The~\envname{proof}~environment automatically places a qed-symbol at its end.
...
\end{proof}
\end{showlatex}
This can be fixed by placing the command~\comname{qedhere}
...
A finer control of tags can be achieved using~\comname{tag} and~\comname{notag}.
...
But if this formula occurs in a multi-line environment like~\envname{align*} then by switching~\envname{align} all lines will be numbered.
